...
//   node scripts/issues.ts triage <file> --work CR-NNNN
//   node scripts/issues.ts resolve <file> --work CR-NNNN --commit <sha> [--note "..."]
...
export function triageIssue(file: string, work: string): void {
  if (!/^CR-\d{4}$/.test(work)) throw new Error('--work must look like CR-0004');
  ...
  try {
    switch (cmd) {
      ...
      case 'list': {
        ...
        for (const r of rows)
          console.log(
            `${r.priority}  ${r.status.padEnd(8)}  ${r.filed}  ${r.work.padEnd(7)}  ${r.file}  ${r.title}`,
          );
        break;
      }
      case 'triage':
        if (!arg || !flags.work) throw new Error('usage: triage <file> --work CR-NNNN');
        triageIssue(resolveFile(arg), flags.work);
        break;
      case 'resolve':
        if (!arg || !flags.work || !flags.commit)
          throw new Error('usage: resolve <file> --work CR-NNNN --commit <sha> [--note "..."]');
