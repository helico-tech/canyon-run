...
function docsTree(files: Record<string, string>): string {
  const docs = fs.mkdtempSync(path.join(os.tmpdir(), 'cr-validate-'));
...
test('reports layout drift and malformed frontmatter', () => {
  const docs = docsTree({
    'plans/x.md': '',
    'notes.md': '',
  ...
  expect(errors).toEqual(
    expect.arrayContaining([
      ...
      expect.stringContaining('work/CR-0001-x.md: id: CR-0002 does not match'),
      expect.stringContaining('work/CR-0001-x.md: status must be'),
      expect.stringContaining('work/CR-0001-x.md: stories need epic'),
      expect.stringContaining('adr/0001-no-date.md: filename must start with'),
    ]),
  );
  expect(errors.filter((e) => e.includes('EPIC-01-y') || e.includes('specs/'))).toEqual([]);
});
