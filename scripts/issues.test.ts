...
test('issue lifecycle: new → list → triage → resolve keeps one file and valid frontmatter', () => {
  const docs = fs.mkdtempSync(path.join(os.tmpdir(), 'cr-docs-'));
  const dir = path.join(docs, 'issues');
  const file = createIssue({
    dir,
    slug: 'Kafka loop',
    title: 'Kafka rebalance loop',
    priority: 'P1',
    date: '2026-09-03',
  });
  expect(path.basename(file)).toBe('2026-09-03-kafka-loop.md');
  expect(() => createIssue({ dir, slug: 'x', title: 'x', priority: 'P9' })).toThrow(/priority/);

  const open = listIssues(dir, 'open');
  expect(open).toHaveLength(1);
  expect(open[0]).toMatchObject({ priority: 'P1', status: 'open', title: 'Kafka rebalance loop' });
  expect(validateDocs(docs)).toEqual([]);

  triageIssue(file, 'CR-0004');
  expect(parseFrontmatter(fs.readFileSync(file, 'utf8'))!.data).toMatchObject({
    status: 'triaged',
    work: 'CR-0004',
  });
  expect(() => triageIssue(file, 'bad')).toThrow(/CR-0004/);

  resolveIssue(file, { work: 'CR-0004', commit: 'abc1234', note: 'fixed', date: '2026-09-04' });
  const doc = parseFrontmatter(fs.readFileSync(file, 'utf8'))!;
  expect(doc.data.status).toBe('resolved');
  expect(doc.body).toContain('**Resolved 2026-09-04** in CR-0004, commit abc1234. fixed');
...
test('lists by priority then filing date', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'cr-issues-'));
