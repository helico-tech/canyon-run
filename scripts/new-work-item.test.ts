...
function tmp(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cr-work-'));
}

test('allocates the next zero-padded id from existing files', () => {
  const dir = tmp();
  expect(nextWorkItemId(dir)).toBe('CR-0001');
  fs.writeFileSync(path.join(dir, 'CR-0007-x.md'), '');
  fs.writeFileSync(path.join(dir, 'EPIC-01-y.md'), '');
  expect(nextWorkItemId(dir)).toBe('CR-0008');
});
...
test('creates a story with valid frontmatter and slugified name', () => {
  ...
  expect(path.basename(file)).toBe('CR-0001-hello-world.md');
