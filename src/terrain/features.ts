...
export interface Feature {
  ...
  z: number;
  r: number;
  /** Arch ring radius; unused otherwise. */
  big: number;
  ...
  if (p.pillarProb > 0) {
    ...
    for (let gz = gz0; gz <= gz1; gz++) {
      for (let gx = gx0; gx <= gx1; gx++) {
        ...
        out.push({ kind: FEATURE_PILLAR, x: px, y: 0, z: pz, r, big: 0, reach: fr });
      }
    }
  }
  if (p.boulderProb > 0) {
...
/** Signed distance to the nearest gathered feature (rock inside), clamped to FEATURE_CLAMP. */
